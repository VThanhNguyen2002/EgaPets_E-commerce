...
export default function CatFoodSection() {
    ...
    const renderCard = (p) => (_jsx(ProductCard, { data: p, onQuickView: () => openQuick(p), onCompare: () => toggleCompare(p) }, p.id));
    return (_jsx("section", { className: styles.sectionWrap, children: _jsxs("div", { className: styles.section, children: [_jsx("h2", { className: sharedStyles.sectionTitle, children: "Dinh d\u01B0\u1EE1ng cho m\u00E8o \uD83D\uDC3E" }), _jsx("div", { className: styles.tabButtons, children: TABS.map((t) => (_jsx("button", { className: activeTab === t.key ? styles.activeTab : "", onClick: () => setActiveTab(t.key), children: t.label }, t.key))) }), _jsx("div", { className: styles.grid, children: loading ? (_jsx("p", { style: { gridColumn: "1/-1" }, children: "\u0110ang t\u1EA3i\u2026" })) : products.length ? (products.slice(0, 6).map(renderCard)) : (_jsx("p", { style: { gridColumn: "1/-1" }, children: "Ch\u01B0a c\u00F3 s\u1EA3n ph\u1EA9m." })) }), _jsx(QuickViewModal, { product: current, isOpen: isQuick, onClose: () => setIsQuick(false) }), _jsx(CompareBar, { compareList: compare, onRemoveItem: (id) => setCompare(compare.filter((x) => x.id !== id)), onClearAll: () => setCompare([]), onCompareNow: () => alert("So sánh ngay!") })] }) }));
}
